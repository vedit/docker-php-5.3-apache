...
##<gpg>##
RUN apt-get update && apt-get install -y gpg gnupg gpg-wks-server gpg-wks-client gnupg-utils dirmngr --no-install-recommends && rm -rf /var/lib/apt/lists/*
...
RUN echo "default_charset = " > $PHP_INI_DIR/php.ini \
      && echo "date.timezone = Etc/GMT+3" >> $PHP_INI_DIR/php.ini
